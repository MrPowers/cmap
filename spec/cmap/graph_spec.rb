...
module Cmap; describe JobRunner do
  ...
  context "#vertices_execution_path" do
    it "organizes simple execution paths" do
      ...
      expect(graph.send(:vertices_execution_path)).to eq ["human_lab_data", "american", "kids", "seniors", "senior_americans"]
    end
    ...
    it "organizes complicated execution paths" do
      ...
      expect(graph.send(:vertices_execution_path)).to eq expected
    end
  end
