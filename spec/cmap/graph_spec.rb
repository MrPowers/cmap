...
module Cmap; describe Graph do
  ...
  context "#vertices_execution_path" do
    it "organizes simple execution paths" do
      ...
      expect(graph.send(:sorted_vertices)).to eq ["human_lab_data", "american", "kids", "seniors", "senior_americans"]
    end
    ...
    it "organizes complicated execution paths" do
      ...
      expect(graph.send(:sorted_vertices)).to eq expected
    end
  end
